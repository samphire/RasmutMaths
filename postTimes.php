...
$sql = "SELECT `time` from `sammath`.`user_tt_times` where `user_id`=" . $_GET['userid'] . " and `number`=" . $_GET['number'];
...
if (mysqli_affected_rows($conn) > 0) { // record exists, so check if time is faster
    if ((int) $_GET['time'] < $result->fetch_row()[0]) { // if true, time is faster, so update
        $sql = "UPDATE `sammath`.`user_tt_times` SET `time` = '" . $_GET['time'] . "' WHERE (`user_id` = '" . $_GET['userid'] . "') and (`number` = '" . $_GET['number'] . "')";
        $result = mysqli_query($conn, $sql);
...
$sql = "UPDATE `sammath`.`tbl_user` SET `avge_time` = '$value' WHERE (`id` = '" . $_GET['userid'] . "')";
...
// print "\naffected rows: " . mysqli_affected_rows($conn);
    ...
    $sql = "UPDATE `sammath`.`tbl_user` SET `perc_tt_done` = '$value' WHERE (`id` = '" . $_GET['userid'] . "')";
    mysqli_query($conn, $sql);
//print "\naffected rows: " . mysqli_affected_rows($conn);
}
...
if ($_GET['op'] == 8) {
    $sql = "UPDATE `sammath`.`tbl_user` SET `perc_tt_passed_at_mark1` = '$value' WHERE `id`=" . $_GET['userid'];
    mysqli_query($conn, $sql);
}

// ####### UPDATE PASSED AT MARK 2 #######
if ($_GET['op'] == 10) {
    $sql = "UPDATE `sammath`.`tbl_user` SET `perc_tt_passed_at_mark2` = '$value' WHERE `id`=" . $_GET['userid'];
    mysqli_query($conn, $sql);
}

// ####### UPDATE PASSED AT 1 MONTH #######
if ($_GET['op'] == 12) { // 1 month - final test
    $sql = "UPDATE `sammath`.`tbl_user` SET `perc_tt_passed_at_1month` = '$value' WHERE `id`=" . $_GET['userid'];
    mysqli_query($conn, $sql);
}
